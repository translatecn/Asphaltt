...
// ListenTCPWithDevice construsts a new TCP listener socket with
// IP_TRANSPARENT option and binds with interface `device`.
func ListenTCPWithDevice(device, network string, laddr *net.TCPAddr) (net.Listener, error) {
	return listenTCP(device, network, laddr)
}
...
func (conn *Conn) DialOriginalDestination(dontAssumeRemote bool) (*net.TCPConn, error) {
	...
	raddr := conn.LocalAddr().(*net.TCPAddr)
	return dialTCP("", laddr, raddr, dontAssumeRemote)
}

// DialOriginalDestinationWithDevice will open a
// TCP connection to the original destination
// that the client was trying to connect to before
// being intercepted.
//
...
func (conn *Conn) DialOriginalDestinationWithDevice(device string, dontAssumeRemote bool) (*net.TCPConn, error) {
	laddr := conn.RemoteAddr().(*net.TCPAddr)
	raddr := conn.LocalAddr().(*net.TCPAddr)
	return dialTCP(device, laddr, raddr, dontAssumeRemote)
}
...
func DialTCP(laddr, raddr *net.TCPAddr) (*net.TCPConn, error) {
	return dialTCP("", laddr, raddr, false)
}

// DialTCPWithDevice will open a
// TCP connection to the specified destination
// with the specified local address, and will bind with interface `device`.
func DialTCPWithDevice(device string, laddr, raddr *net.TCPAddr) (*net.TCPConn, error) {
	return dialTCP(device, laddr, raddr, false)
}

func dialTCP(device string, laddr, raddr *net.TCPAddr, dontAssumeRemote bool) (*net.TCPConn, error) {
