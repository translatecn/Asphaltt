...
// /sys/kernel/debug/tracing/events/netlink/netlink_extack/format
...
// `xdpsnoop` is an eBPF-based tool to trace XDP program installation and removal on NIC drivers.
// It is useful when something bad happens to the XDP program installation, and some error message will be printed if provided.
