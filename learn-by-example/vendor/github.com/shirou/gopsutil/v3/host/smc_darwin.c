...
static kern_return_t call_smc(SMCParamStruct *input, SMCParamStruct *output) {
    ...
    result = IOConnectCallStructMethod(conn, kSMCHandleYPCEvent, input, input_cnt, output, &output_cnt);
...
double gopsutil_v3_get_temperature(char *key) {
    ...
    if (!(result == kIOReturnSuccess) && result_smc.data_size == 2 && result_smc.data_type == to_uint32(DATA_TYPE_SP78)) {
        return 0.0;
    }
