...
static __always_inline void handle_event(void *ctx, struct event *event) {
...
static __always_inline int devinet_conf_ifindex(struct net *net, struct ipv4_devconf *cnf) {
...
static __always_inline int __fexit(void *ctx, struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos, int retval) {
...
int BPF_PROG(fexit_devinet_conf_proc, struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos, int retval) {
    return __fexit(ctx, ctl, write, buffer, lenp, ppos, retval);
}

SEC("fexit/ipv4_doint_and_flush")
int BPF_PROG(fexit_ipv4_doint_and_flush, struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos, int retval) {
    return __fexit(ctx, ctl, write, buffer, lenp, ppos, retval);
}

SEC("fexit/devinet_sysctl_forward")
int BPF_PROG(fexit_devinet_sysctl_forward, struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos, int retval) {
    return __fexit(ctx, ctl, write, buffer, lenp, ppos, retval);
}