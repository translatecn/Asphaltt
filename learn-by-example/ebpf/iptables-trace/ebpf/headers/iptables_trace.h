...
static __always_inline bool filter_l3_and_l4_info(struct sk_buff *skb) {
    u32 addr = cfg->ip;
    u8 proto = cfg->proto;
    u16 port = cfg->port;
...
static __always_inline bool filter_netns(struct sk_buff *skb) {
    u32 netns = get_netns(skb);
    return cfg->netns && netns && cfg->netns != netns;
}

static __always_inline bool filter_pid(u32 pid) {
